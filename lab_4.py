...
x_0 = float(input('Введите начальное значение аргумента: '))
x_n = float(input('Введите конечное значение аргумента: '))
...
while i < x_n + eps:
	...
	if abs(i) < eps: # проблемы с выводом 0 у вещественных чисел
		print('| {: <9.4} |  {: <9.4} |'.format(0.0, y))
	else:
		print('| {: <9.4} |  {: <9.4} |'.format(i, y))
	i += h
...
ox = 11 + (int(width/(y_mx - y_mn) * (0 - y_mn)) - 1)
...
while i < x_n + eps:
	...
	if y_mn <= 0 <= y_mx:
		...
		if abs(i) < eps:
			r = '{:<9.4}|'.format(0.0) + (int(k1 * (y - y_mn)) - 1)*' ' + '*' + k2*' '
		else:
			r = '{:<9.4}|'.format(i) + (int(k1 * (y - y_mn)) - 1)*' ' + '*' + k2*' '
		if r[ox] != '*':
			r = r[:ox] + '|' + r[ox + 1:]
	...
	# вывод графика, непересекающего ocь X
		...
		if abs(i) < eps:
			print('{:<9.4}|'.format(0.0) + (int(k1 * (y - y_mn)) - 1)*' ' + '*' +  k2*' ')
		else:
			
			print('{:<9.4}|'.format(i) + (int(k1 * (y - y_mn)) - 1)*' ' + '*' +  k2*' ')

	i += h

# вывод суммы и произведения y
print('Сумма y: {:0.4}'.format(summ))
print('Произведение y: {:0.4}'.format(mult))